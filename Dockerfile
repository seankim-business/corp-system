...
CMD ["sh", "-c", "echo '=== Starting Nubabel Container v5 ===' && echo 'Environment: '${NODE_ENV} && echo '' && echo '=== Resolving Failed Migration ===' && (npx prisma migrate resolve --rolled-back 20260126_add_feature_flags --schema=prisma/schema.prisma 2>/dev/null || echo 'Already resolved or no failed migration') && echo '' && echo '=== Running Database Migrations ===' && (npx prisma migrate deploy --schema=prisma/schema.prisma && echo '✅ Migrations applied successfully' || echo '⚠️  Migration failed, trying db push...') && (npx prisma db push --accept-data-loss --schema=prisma/schema.prisma 2>/dev/null || true) && echo '' && echo '=== Starting Node.js Server ===' && node dist/index.js"]
